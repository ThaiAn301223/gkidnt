...
import 'package:ungdunggiuaki/product.dart';
import 'package:ungdunggiuaki/add_product.dart';
    ...
    _productRef.onValue.listen((event) {
      ...
      if (data != null) {
        data.forEach((key, value) {
          products.add(Product.fromMap(value, key)); // Sử dụng fromMap để tạo sản phẩm
        });
      }
      ...
      print("Products fetched: ${_products.length}"); // Kiểm tra số lượng sản phẩm
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                ...
                if (result == true) {
                  _fetchProducts(); // Cập nhật lại danh sách sản phẩm khi quay lại nếu có thay đổi
                }
              });
            },
          ),
      ...
      body: _products.isEmpty
          ? const Center(child: Text('Không có sản phẩm nào')) // Hiển thị khi không có sản phẩm
          : ListView.separated(
              separatorBuilder: (context, index) => const Divider(), // Thêm dòng phân cách giữa các sản phẩm
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  leading: product.imageUrl.isNotEmpty
                      ...
                      : const Icon(Icons.image, size: 50), // Hiển thị biểu tượng nếu không có hình ảnh
                  trailing: Row(
                    mainAxisSize: MainAxisSize.min, // Đảm bảo kích thước nhỏ gọn của Row
                    children: [
                      IconButton(
                        icon: const Icon(Icons.edit, color: Colors.blue), // Biểu tượng chỉnh sửa
                        onPressed: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                            ...
                            if (result == true) {
                              _fetchProducts(); // Cập nhật lại danh sách sản phẩm khi có thay đổi
                            }
                          });
                        },
                      ),
                      IconButton(
                        icon: const Icon(Icons.delete, color: Colors.red), // Biểu tượng xóa
                        onPressed: () {
                          _showDeleteConfirmationDialog(product); // Hiển thị hộp thoại xác nhận
                        },
                      ),
                    ],
                  ),
